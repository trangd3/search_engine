...
if __name__ == "__main__":
    ...
    for word in query: 
        if words[word]:
            query_tfidf[word] = (1 + math.log(query_frequencies[word], 10)) * words[word]["idf"]
        ...
        for word in query:
            ...
            try:
                ...
                if words[word]["docId"][doc]["bolded"]:
                    multiplier += 0.1
                norm = words[word]["docId"][doc]["tfidf"] * multiplier / doc_length
                doc_norms.append(norm)
            except KeyError:
                doc_norms.append(0)

        doc_scores[doc] = float(np.dot(query_norms, doc_norms))
    print(sorted(doc_scores.items(), key = lambda kv: -kv[1])[:100])
